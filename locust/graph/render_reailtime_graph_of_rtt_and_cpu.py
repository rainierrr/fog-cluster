...
try:
    while True:
        ...
        for mg_node_dict in mg_node_list:
            cluster_metrics = fog_client.get_cluster_metrics(
                mg_node_dict['ip'])
        ...
        cpu_ax.set_xlim(start_index, x_length*5)
        rtt_ax.set_xlim(start_index, x_length*5)
    ...
    print(e)
    print("Error: 例外が発生しました。")

    with open('rtt_and_cpu.csv', 'a') as f:
        writer = csv.writer(f)
        for i in range(len(x)):
            writer.writerow([x[i], cluster_a_cpu_y[i], cluster_b_cpu_y[i],
                            cluster_a_rtt_y[i], cluster_b_rtt_y[i]])
